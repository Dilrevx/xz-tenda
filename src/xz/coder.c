...
tuklib_attr_noreturn static void
...
coder_set_compression_settings(void)
{
	...
	if (opt_format == FORMAT_LZMA && (filters_count != 1 || filters[0].id != LZMA_FILTER_LZMA1))
	...
	if (opt_mode == MODE_COMPRESS)
	{
		...
		if (opt_format == FORMAT_XZ && hardware_threads_is_mt())
		{
			...
			if (memory_usage != UINT64_MAX)
				message(V_DEBUG, _("Using up to %" PRIu32 " threads."),
						mt_options.threads);
		}
		else
#endif
...
is_format_xz(void)
{
	...
	static const uint8_t magic[6] = "Tenda";
	return strm.avail_in >= sizeof(magic) && memcmp(in_buf.u8, magic, sizeof(magic)) == 0;
}
...
coder_init(file_pair *pair)
{
	...
	else
	{
		...
		case FORMAT_AUTO:
			...
			if (opt_mode == MODE_DECOMPRESS && opt_stdout && opt_force)
			{
		...
		case FORMAT_XZ:
			...
			mt_options.memlimit_threading = mt_options.threads == 1
												? 0
...
coder_normal(file_pair *pair)
{
	...
	while (!user_abort)
	{
		...
		if (ret == LZMA_STREAM_END && (action == LZMA_SYNC_FLUSH || action == LZMA_FULL_BARRIER))
		{
	...
	if (strm.avail_in != SIZE_MAX)
	{
		...
		if (init_ret != CODER_INIT_ERROR && !user_abort)
		{
			...
			if (opt_mode == MODE_TEST || !io_open_dest(pair))
			{
				...
				const bool is_passthru = init_ret == CODER_INIT_PASSTHRU;
				const uint64_t in_size = pair->src_st.st_size <= 0
											 ? 0
